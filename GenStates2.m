...
a=2;
% for i=1:size(indprobpartial,1)
%     if indprobpartial(i,1)=indprobpartial
%     indprobfinal(i,1)
% end
...
for i=1:Gnum
    ...
    gcopt(1,1)=states(1,1);
    a=2;
    b=2;
    gcopt(1,2)=states(1,2);
    for i=2:size(table,1)
        if (states(i,1)==states(i-1,1))
            gcopt(b-1,2)=gcopt(b-1,2)+states(i,2);
        else
            gcopt(a,1)=states(i,1);          
            gcopt(b,2)=states(i,2);
            b=b+1;
            a=a+1;           
        end
    end
